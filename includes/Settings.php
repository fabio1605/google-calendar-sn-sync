...
if ($token_row) {
    ...
    $expires_in = (int) $token_row->expires_in;
    $now = time();

    if (($created + $expires_in - 300) < $now) { // 5 min early refresh
        // Token expired → Refresh it
        error_log('⚡ Access token expired during settings page load. Attempting to refresh...');
        ...
        if (!is_wp_error($response)) {
            ...
            if (!empty($data['access_token'])) {
                ...
                error_log('✅ Access token successfully refreshed during settings page load.');

                $access_token = $data['access_token']; // Update for immediate use
            } else {
                error_log('❌ Failed to refresh token: ' . wp_remote_retrieve_body($response));
            }
        } else {
            error_log('❌ HTTP error during token refresh: ' . $response->get_error_message());
        }
    }
}
